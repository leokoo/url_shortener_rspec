...
RSpec.describe Url, type: :model do
	...
	context "self.retrive_short_url: " do
		it "takes in one valid long_url" do
			expect{ Url.retrive_short_url(proper_long_url) }.not_to raise_error
		end

		it "returns nil when proper long_url is not found" do
			expect( Url.retrive_short_url(proper_long_url) ).to be nil
		end

		it "returns short_url when proper long_url is found" do
			Url.create(long_url: proper_long_url, short_url: proper_short_url)
			expect( Url.retrive_short_url(proper_long_url) ).to eq proper_short_url
		end
	end
